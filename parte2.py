...
def conjunto_palavras_para_cadeia(c):
	...
	def ordena_subconjunto(sub):
		'''ordena um subconjunto de um conjunto_palavras
		ordena_subconjunto: lista -> lista'''
	...
	if e_conjunto_palavras(c):
		...
		return filtra_plicas(cad) # retira as plicas
...
def adiciona_palavra_valida(jog, palavra):
	...
	if e_jogador(jog) and e_palavra_potencial(palavra):
		# so atualiza a lista e a pontuacao se nao tiver sido descoberta 
...
def adiciona_palavra_invalida(jog, palavra):
	...
	if e_jogador(jog) and e_palavra_potencial(palavra):
		# so atualiza a lista e a pontuacao se nao tiver sido utilizada anteriormente
...
def guru_mj(letras):
	...
	def copia_lista(lista):
		'''funcao auxiliar
		copia os elementos de uma lista para uma nova
	...
	print(letras)
	jogadores = inscricao() # recebe os jogadores
	palavras_validas = gera_todas_palavras_validas(letras) # gera as palavras validas
	por_descobrir = copia_lista(palavras_validas) # replica as palavras validas
	jogada = 0 
	n_jog = len(jogadores) # numero de jogadores

	while not ganho(por_descobrir):
		print("JOGADA", jogada + 1, "- Falta descobrir", numero_palavras(por_descobrir), "palavras")
		proposta = recebe_proposta(jogadores[jogada % n_jog], letras)
		cad = palavra_potencial_para_cadeia(proposta) + " - palavra " # cadeia a imprimir

	# verifica a validade da proposta e se e repetida
		if pertence_conjunto(palavras_validas, proposta):
			print(cad + "VALIDA")
			if pertence_conjunto(por_descobrir, proposta):
				adiciona_palavra_valida(jogadores[jogada % n_jog], proposta)
				remove_palavra(por_descobrir, proposta)
		else:
			print(cad + "INVALIDA")
			adiciona_palavra_invalida(jogadores[jogada % n_jog], proposta)

		jogada = jogada + 1

	venc = vencedores(jogadores) # calcula os vencedores
	if len(venc) == 1:
